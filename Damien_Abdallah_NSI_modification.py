...
# ajout d'un scientifique et des informations le concernant
	nouvelle_ligne = nom.split(", ")
	with open(r"U:\damien.decarre\scientifiques.csv",'a', newline='', encoding="utf-8") as file:
...
#suppression d'un scientifique du fichier scientifiques
    ...
    # stocke le fichier scientifiques dans une liste
        reader = csv.reader(file, delimiter=";")
        for row in reader:
            scientifiques.append(row)
        ...
        for row in scientifiques:
            if row != []:
                if row[0] != nom:
                    writer.writerow(row)
    decouvreurs = []
    with open(r"U:\damien.decarre\decouvreurs.csv", newline="", encoding="utf-8") as file:
    # stocke le fichier decouvreurs dans une liste
        reader = csv.reader(file, delimiter=";")
        for row in reader:  			
            decouvreurs.append(row)
...
# ajout d'une decouverte et des informations le concernant
	nouvelle_ligne = denomination.split(", ")
	with open(r"U:\damien.decarre\decouvertes.csv",'a', newline='', encoding="utf-8") as file:
		writer = csv.writer(file, delimiter=";")
		writer.writerow(nouvelle_ligne)
...
#suppression d'une découverte du fichier decouvertes
    ...
    # stocke le fichier decouvertes dans une liste
        reader = csv.reader(file, delimiter=";")
        for row in reader:
            decouvertes.append(row)
...
# ajout d'une relation si le scientifque et la découverte existent déjà
    ...
    if existe_deouverte and existe_decouverte:	
        with open(r"U:\damien.decarre\decouvreurs.csv",'a', newline='', encoding="utf-8") as file:
            writer = csv.writer(file, delimiter=";")
            writer.writerow(nouvelle_ligne)
